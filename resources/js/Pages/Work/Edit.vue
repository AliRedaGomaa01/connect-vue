...
let form = useForm({
    type:  Object.keys(props.types).includes(props.work.category) ? props.work.category : 'Other',
    category: props.work.category,
    title: props.work.title,
    description: props.work.description,
    url: props.work.url,
});
let showCategory = ref(form.type == "Other");
let setCategory = () => {
...
<template>
    <Head :title="isEn ? 'Edit Work' : 'تعديل عمل'"/>
    <div class="container">
        <form @submit.prevent="submit">
            <div class="grid gap-5 m-5 p-5 my-shadow rounded-xl center-container">
                ...
                <PrimaryButton type="submit" class="justify-self-end">{{ isEn ? "Edit" : "تعديل" }}</PrimaryButton>
            </div>
        </form>
    </div>
</template>
