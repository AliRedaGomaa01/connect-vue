...
<template>
    ...
    <div class="container">
        <Link href="{{route('users.search')}}" class="my-shadow rounded-xl m-5 p-5 w-[90%] grid gap-5">
                <PrimaryButton class="justify-self-center">{{ $page.props.isEn ? 'Search Page' : 'صفحة البحث'}}</PrimaryButton>
        </Link>
        <div class="" v-if="users != false">
            <div class="my-shadow rounded-xl m-5 p-5 w-[90%] grid gap-5" v-for="user in users" :key="user.id">
                ...
                <a href="{{route('users.show',user.id)}}" class="underline justify-self-center" :class='contentClasses'><PrimaryButton class="">{{$page.props.isEn? 'Show' : 'اعرض'}}</PrimaryButton></a>
            </div>
        </div>
        <!-- buttons -->
        <div class="grid grid-cols-3 it-ce w-full my-10 " v-if="pages.last !== 1">
